...
//variables relevant to running average arrays for analog and digital
float analog_avg_hrArray[30];
float analog_avg_hr;
int a = 0;
int j; 

float digital_avg_hrArray[30];
float digital_avg_hr; 
int d = 0;
int k; 
...
void analogHRMonitor(unsigned long currentAnalogMillis) {
  ...
  if (voltage > analog_threshold) {
    ...
    if (analog_beat_counter >= 2) {
      ...
      analog_avg_hrArray[a] = analog_inst_hr[analog_inst_hr_count]; 
      a++;

      if ((a%30)==0){                     //if remainder of i/30 is 0 then you reset the array; (reset i to refill the first cell after cells exceed 60)
        a=0;
      }
      for (j=0; j<30; j++){
       analog_avg_hr += analog_avg_hrArray[j];   //calculate one sum over entire array
      }
      
      
      Serial.print("Analog IHR: "); 
  ...
  if (currentAnalogMillis - startTime >= (MIN_IN_MILLIS)) {
    ...
    float analog_avg_hr_val = analog_avg_hr/30;
...
void digitalHRMonitor(volatile byte digitalSignalState, unsigned long currentDigitalMillis) {
  if (digitalSignalState  != HIGH) {
    ...
    if (digital_beat_counter >= 2) {
      ...
      Serial.println(digital_inst_hr_count); 
      //calculate instantaneous heart rate
      ...
      digital_avg_hrArray[d] = digital_inst_hr[digital_inst_hr_count];
      d++;
      
      if ((d%30)==0){                     //if remainder of i/30 is 0 then you reset the array; (reset i to refill the first cell after cells exceed 60)
        d=0;
      }
      for (k=0; k<30; k++){
       digital_avg_hr += digital_avg_hrArray[j];   //calculate one sum over entire array
      }
      
      Serial.print("Digital IHR: "); 
  ...
  if (currentDigitalMillis - startTime >= (MIN_IN_MILLIS)) {
    float digital_avg_hr_val = digital_avg_hr/30; 
    Serial.print("Digital Average: "); 
